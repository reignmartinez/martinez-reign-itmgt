...
def shift_by_letter(letter, letter_shift):
    ...
    new_pos = (original_pos + shift_amount) % 26
...
def vigenere_cipher(message, key):
    ...
    for char in message:
        if char == " ":
            ...
            list_message.append(ord(char))
    if len(list_message) <= len(list_key):
        for char in key:
            list_key.append(ord(char) - ord('A'))
    else: 
    ...
    for i in range(0,len(list_message)):
        if list_message[i] == " ":
            list_encrypted.append(" ")
        else:
            list_encrypted.append(chr(zero + ((list_message[i] - zero + (list_key[i])) % 26)))
